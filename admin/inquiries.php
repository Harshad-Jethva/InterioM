...
<head>
    <meta charset="UTF-8">
    <title>Manage Inquiries - Spaces by KD</title>
        ...
        <div class="bg-[#1a1a1a] text-white w-64 flex-shrink-0 flex flex-col">
            ...
            <nav class="flex-1 overflow-y-auto py-4">
                <ul class="space-y-2 px-4">
                    ...
                    <li><a href="inquiries.php" class="flex items-center space-x-3 bg-gray-700/50 text-[#c0a062] px-4 py-2 rounded-md"><i class="fas fa-envelope-open-text w-5"></i><span>Inquiries</span></a></li>
                    <li><a href="users.php" class="flex items-center space-x-3 text-gray-300 hover:bg-gray-800 hover:text-white px-4 py-2 rounded-md"><i class="fas fa-users-cog w-5"></i><span>User Roles</span></a></li>
                </ul>
            </nav>
        </div>
                        ...
                        <?php if(count($inquiries) > 0): ?>
                            ...
                            <tr>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 font-medium"><?php echo htmlspecialchars($i['name']); ?></p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 text-xs"><?php echo htmlspecialchars($i['contact_info']); ?></p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-500 text-xs mt-1 max-w-xs"><?php echo htmlspecialchars($i['message']); ?></p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <span class="relative inline-block px-3 py-1 font-semibold leading-tight <?php echo $i['status'] == 'new' ? 'text-blue-900' : 'text-green-900'; ?>">
                                        <span aria-hidden="true" class="absolute inset-0 <?php echo $i['status'] == 'new' ? 'bg-blue-200' : 'bg-green-200'; ?> opacity-50 rounded-full"></span>
                                        <span class="relative"><?php echo ucfirst($i['status']); ?></span>
                                    </span>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap"><?php echo date('M j, Y', strtotime($i['created_at'])); ?></p>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="5" class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center">No inquiries yet.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
